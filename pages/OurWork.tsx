...
const OurWork: React.FC = () => {
    ...
    return (
        <div className="bg-brand-dark">
            <SEO
                pageType="service"
                title="Our Work Portfolio | The Shop Autobody Burnaby"
                description="Browse our gallery of high-quality collision repair, auto painting, and dent removal projects at The Shop Autobody in Burnaby, BC."
            />
            {isLoggedIn && (
            ...
            <div className="container mx-auto px-6 py-16">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {gallery.map((item, index) => (
                        <div
                            key={index}
                            className={`group bg-brand-gray rounded-lg shadow-lg overflow-hidden flex flex-col transition-all duration-500 ease-in-out transform border border-transparent hover:border-brand-blue hover:-translate-y-2 ${itemBeingRemoved?.type === 'work' && itemBeingRemoved.index === index ? 'opacity-0 scale-75' : 'opacity-100 scale-100'
                                }`}
                        >
                ...
                <div className="mt-20">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                        {testimonials.map((testimonial, index) => (
                            <div
                                key={index}
                                className={`bg-brand-gray p-8 rounded-lg shadow-lg flex flex-col transition-all duration-500 ease-in-out transform border border-transparent hover:border-brand-blue hover:-translate-y-2 ${itemBeingRemoved?.type === 'testimonial' && itemBeingRemoved.index === index ? 'opacity-0 scale-75' : 'opacity-100 scale-100'
                                    }`}
                            >
