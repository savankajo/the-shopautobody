  ...
  return (
    <header className="bg-brand-gray/95 backdrop-blur-md border-b border-brand-light-gray sticky top-0 z-50 h-20 md:h-28 flex items-center shadow-xl">
      <nav className="container mx-auto px-6 flex items-center justify-between h-full">
        {/* Logo Section */}
        <Link to="/" className="flex items-center h-full py-2 shrink-0">
          {businessInfo.logoUrl && (
            <img
              src={businessInfo.logoUrl}
              className="h-12 md:h-20 lg:h-24 max-w-[150px] md:max-w-none w-auto object-contain transition-transform duration-300 hover:scale-105"
          ...
          <div className="flex items-center space-x-8">
            {navLinks.map((link) => (
              <NavLink
                ...
                className={({ isActive }) =>
                  `block py-1 transition duration-150 ease-in-out text-base font-bold tracking-wide ${isActive ? 'text-brand-blue border-b-2 border-brand-blue' : 'text-text-body hover:text-brand-blue'
                  }`
                }
              >
                {link.name}
            ...
            {navLinks.map((link) => (
              <li key={link.name}>
                <NavLink
                  ...
                  className={({ isActive }) =>
                    `block py-4 px-6 rounded-2xl transition duration-150 ease-in-out text-xl font-bold ${isActive ? 'bg-brand-blue text-white shadow-lg' : 'text-text-body bg-brand-light-gray/10'
                    }`
                  }
                >
                  {link.name}
